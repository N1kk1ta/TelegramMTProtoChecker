...
def normalize_secret_for_mtp(secret):
    """Normalize a Telegram proxy secret for the bundled mtp_ping.

    IMPORTANT: mtp_ping treats a secret beginning with the character ``7`` as
    URL-safe Base64. Therefore Base64 detection must happen BEFORE the 32-hex
    check; a 32-character Base64 string can consist entirely of hex-looking
    characters and would otherwise be misclassified as a normal secret.

    Accepted values:
      - 32 hex chars: normal MTProto (16-byte secret)
      - dd + 32 hex chars: secure/padded transport
      - ee + 32+ hex chars: Fake-TLS hex
      - Base64/Base64URL starting with ``7`` and decoding to 16 bytes: normal
      - Base64/Base64URL starting with ``7`` and decoding to DD + 16 bytes:
        secure
      - Base64/Base64URL starting with ``7`` and decoding to EE + 16 bytes
        + optional SNI: Fake-TLS

    Anything else is rejected locally and is never passed to mtp_ping. This
    prevents mtp_ping's ``unexpected base64 secret value`` error for values
    such as an EF-prefixed decoded Base64 secret.
    """
    import base64
    ...
    # mtp_ping's own parser explicitly recognizes Base64 by a leading '7'.
    # This check MUST precede the hex check because a Base64 string can also
    # contain only [0-9a-f] characters (for example, a 32-char value).
    if s.startswith("7"):
        if not re.fullmatch(r"[A-Za-z0-9+/_-]+={0,2}", s):
            raise ValueError("Некорректный Base64 secret")
        core=s.rstrip("=")
        if len(s) % 4 == 1 or len(s) - len(core) > 2:
            raise ValueError("Некорректный Base64 secret")
        try:
        ...
        prefix=raw[:1].hex().upper() if raw else "--"
        raise ValueError(
            f"Некорректный Base64 secret: {len(raw)} байт, префикс {prefix}; "
            "ожидался 16-байтный secret или DD/EE-secret"
        )

    # Canonical hexadecimal formats.
    ...
    # Non-7 Base64 can still be a valid 16-byte normal secret. Decode it and
    # convert valid MTProto byte layouts to canonical hex before invoking
    # mtp_ping. The 7-prefix special case above is needed to disambiguate
    # Base64 strings that also happen to look like 32 hex characters.
...
def ptype(secret):
    ...
    if re.fullmatch(r"[A-Za-z0-9+/_-]+={0,2}", s):
        try:
            ...
            if len(raw) >= 17 and raw[0] == 0xEE:
                return "Fake-TLS / EE / Base64"
        except Exception:
            pass
        # If it is exactly 32 hex characters, the canonical hex branch below
        # is authoritative unless it starts with 7 (handled as Base64 above).
        if s.startswith("7"):
            return "Некорректный Base64 secret"
    if re.fullmatch(r"[0-9a-f]{32}", s, re.I):
        return "Normal"
    if re.fullmatch(r"dd[0-9a-f]{32}", s, re.I):
        return "DD / Secure"
    if re.fullmatch(r"ee[0-9a-f]{32,}", s, re.I):
        return "Fake-TLS / EE"
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Telegram Proxy Checker v5.17")
    ...
    def build(self):
        ...
        ttk.Label(self,text="Проверка выполняется через настоящий mtp_ping: MTProto req_pq/res_pq + Telegram ping. В v5.17 Erlang/OTP и mtp_ping упакованы внутрь одного EXE.",padding=10).pack(anchor="w")
