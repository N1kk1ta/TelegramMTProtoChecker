...
def normalize_secret_for_mtp(secret):
    """Normalize a Telegram proxy secret to a canonical form for mtp_ping.

    Accepted canonical formats:
      * 32 hex chars: normal MTProto (16 bytes)
      * dd + 32 hex chars: secure/padded transport
      * ee + 32+ hex chars: Fake-TLS in hex (EE + 16-byte secret + SNI)

    Telegram links may also contain URL-safe Base64.  We decode Base64 only
    when the decoded bytes form one of the valid MTProto secret formats.  We
    NEVER convert arbitrary Base64 bytes to hex: that was the source of the
    previous "unexpected base64 secret value" failures.
    """
    import base64

    s=urllib.parse.unquote(str(secret or "")).strip()
    # parse_qs already converts '+' to a space in query parameters. Restore it
    # for standard Base64 copied into a URL; URL-safe '-'/'_' are unchanged.
    s=s.replace(" ", "+")

    # Canonical hexadecimal forms.
    ...
    # Base64/Base64URL. Padding is optional in Telegram links.
    ...
    # 16 bytes = normal MTProto secret.
    if len(raw) == 16:
        return raw.hex()

    # 17 bytes beginning with DD = secure/padded transport marker + secret.
    if len(raw) == 17 and raw[0] == 0xDD:
        return raw.hex()

    # Fake-TLS Base64 is EE + 16-byte secret + SNI/domain bytes.  Require
    # at least EE + 16-byte secret; any additional bytes are the SNI.
    if len(raw) >= 17 and raw[0] == 0xEE:
        return raw.hex()

    # Do not pass arbitrary decoded bytes to mtp_ping.  In particular, a
    # 23/24-character Base64 value can decode to 17/18 bytes with an unrelated
    # first byte (EF, etc.); that is not a valid Telegram secret format.
    raise ValueError(
        f"Некорректный Base64 secret: декодировано {len(raw)} байт; "
        "ожидалось 16 байт или DD/EE-секрет"
    )

def ptype(secret):
    # Base64 is case-sensitive, so do not lowercase the value before decoding.
    ...
    if re.fullmatch(r"ee[0-9a-f]{32,}", s, re.I):
        return "Fake-TLS / EE"
    if re.fullmatch(r"[a-z0-9+/=_-]+", s, re.I):
        try:
            import base64
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Telegram Proxy Checker v5.16")
    ...
    def build(self):
        ...
        ttk.Label(self,text="Проверка выполняется через настоящий mtp_ping: MTProto req_pq/res_pq + Telegram ping. В v5.16 Erlang/OTP и mtp_ping упакованы внутрь одного EXE.",padding=10).pack(anchor="w")
