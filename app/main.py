...
def normalize_secret_for_mtp(secret):
    """Normalize Telegram MTProto secrets for mtp_ping.

    Telegram proxy links can contain secrets in hex or URL-safe Base64.
    Base64 for a 16-byte secret is 24 chars with ``==`` padding or 22 chars
    without it. A 17-byte secret (usually a leading DD transport marker plus
    the 16-byte secret) is 24 chars with one ``=`` or 23 chars unpadded.
    mtp_ping is most reliable when these are converted to canonical hex.
    Fake-TLS Base64 secrets are also decoded to their canonical hex form when
    possible, preserving the EE prefix and SNI bytes.
    """
    s=urllib.parse.unquote(str(secret or "")).strip()
    # Restore a common URL-safe Base64 variant where spaces were introduced
    # by a badly copied query parameter.
    s=s.replace(" ", "+")

    # Canonical hex forms: normal (16 bytes), secure/DD (17 bytes), and
    # Fake-TLS/EE (17+ bytes including SNI/domain bytes).
    if re.fullmatch(r"[0-9a-fA-F]{32}", s):
        return s.lower()
    if re.fullmatch(r"dd[0-9a-fA-F]{32}", s):
        return s.lower()
    if re.fullmatch(r"ee[0-9a-fA-F]{32,}", s):
        return s.lower()

    # URL-safe Base64, with or without '=' padding.  Valid Base64 secrets
    # encountered in Telegram links decode to at least 16 bytes.  In
    # particular, 22/23/24-character values are NOT automatically rejected:
    # 22 and 24 chars can represent a 16-byte secret, while 23/24 chars can
    # represent a 17-byte secret depending on padding.
    if re.fullmatch(r"[A-Za-z0-9_-]+={0,2}", s):
        try:
            raw=__import__('base64').urlsafe_b64decode(s + "=" * ((-len(s)) % 4))
            if len(raw) == 16:
                return raw.hex()
...
def ptype(secret):
    s=urllib.parse.unquote(str(secret or "")).strip().lower()
    if s.startswith("dd"): return "DD / Secure"
    if s.startswith("ee") or len(s)>32: return "Fake-TLS / EE"
    if re.fullmatch(r"[A-Za-z0-9_-]+={0,2}", s):
        try:
            raw=__import__('base64').urlsafe_b64decode(s + "=" * ((-len(s)) % 4))
            if raw and raw[0] == 0xDD: return "DD / Secure / Base64"
            if raw and raw[0] == 0xEE: return "Fake-TLS / EE / Base64"
            if len(raw) == 16: return "Normal / Base64"
        except Exception:
            pass
    return "Normal"
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Telegram Proxy Checker v5.15")
    ...
    def build(self):
        ...
        ttk.Label(self,text="Проверка выполняется через настоящий mtp_ping: MTProto req_pq/res_pq + Telegram ping. В v5.15 Erlang/OTP и mtp_ping упакованы внутрь одного EXE.",padding=10).pack(anchor="w")
    ...
    def check_one(self,p):
        ...
        # Rebuild the URL with the normalized secret. This fixes the common
        ...
        cmd += ["--timeout",str(self.timeout.get()),"--repeat",str(self.repeat.get()),check_url]
        startupinfo=None
        creationflags=0
        if os.name == "nt":
